...
//bagian untuk tabel
// Column widths
$w = array(8, 25, 25, 25, 35, 10, 85, 30, 30);
/*isi ln dari string
30 = 14
40 = 18
15 = 7
*/
$header = array('No', 'Tanggal Ujian', 'Waktu Ujian', 'Tempat Ujian', 'Nama Mahasiswa', 'Kelas', 'Judul Skripsi', 'Nama Penguji', 'Peran');
	...
	IF($tanggal == $data['tanggal']){
		...
		IF($waktu == $data['waktu']){
			IF($npm == $data['npm']){
				...
				$x += $w['4'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['5'], 5, '', '', 'C', 0);
				$y6 = $pdf->GetY();		
				$x += $w['5'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['6'], 5, '', '', 'L', 0);
				$y7 = $pdf->GetY();		
				$x += $w['6'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['7'], 5, $data['dosen'], 'T', 'L', 0);
				$y8 = $pdf->GetY();		
				$x += $w['7'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['8'], 5, $data['peran'], 'T', 'L', 0);
				$y9 = $pdf->GetY();	
				...
				$x += $w['2'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['3'], 5, $data['ruang'], 'T', 'C', 0);
				$y4 = $pdf->GetY();		
				$x += $w['3'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['4'], 5, $data['nama'], 'T', 'L', 0);
				$y5 = $pdf->GetY();		
				$x += $w['4'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['5'], 5, $data['kelas'], 'T', 'C', 0);
				$y6 = $pdf->GetY();		
				$x += $w['5'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['6'], 5, ucwords(strtolower($data['judul'])), 'T', 'L', 0);
				$y7 = $pdf->GetY();		
				$x += $w['6'];
				$pdf->SetXY($x, $y);
				$pdf->MultiCell($w['7'], 5, $data['dosen'], 'T', 'L', 0);
			...
			$x += $w['1'];
			$pdf->SetXY($x, $y);
			$pdf->MultiCell($w['2'], 5, $data['waktu'], 'T', 'C', 0);
			$x += $w['2'];
			$pdf->SetXY($x, $y);
			$pdf->MultiCell($w['3'], 5, $data['ruang'], 'T', 'C', 0);
			$y4 = $pdf->GetY();		
		...
		$x += $w['0'];
		$pdf->SetXY($x, $y);
		$pdf->MultiCell($w['1'], 5, nama_hari(DATE('w', strtotime($data['tanggal']))).', '.DATE('d-m-Y', strtotime($data['tanggal'])), 'T', 'L', 0);
		$x += $w['1'];
		$pdf->SetXY($x, $y);
		$pdf->MultiCell($w['2'], 5, $data['waktu'], 'T', 'C', 0);
		$x += $w['2'];
