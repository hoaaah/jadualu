...
//			echo CHtml::activeDropDownList($model, 'category_id', CHtml::listData(RefCategory::model()->findAll(), 'id', 'name'));
		  echo CHtml::dropDownList('Tfiles[bidang_id]','', 
		 CHtml::listData(RefBidang::model()->findAll(), 'id', 'name'),
		  array(
			...
			'update'=>'#Tfiles_category_id', //or 'success' => 'function(data){...handle the data in the way you want...}',
		  'data'=>array('bidang_id'=>'js:this.value'),
		  ))); 
			echo CHtml::dropDownList('Tfiles[category_id]','', array(), array('prompt'=>'Pilih Kategori'));		  
		}ELSE{
	...
	<div class="row">
		<?php echo $form->labelEx($model,'no', array('label' => 'Nama Files, hanya boleh numerik dan titik!')); ?>
		<?php echo $form->textField($model,'no',array('prompt'=>'Hanya boleh numerik dan titik.', 'size'=>60,'maxlength'=>100)); ?>
	...
	<div class="row">
     ...
     <div class="row">
        <?php echo CHtml::activeLabelEx($model,'tentang', array('label' => 'diisi dengan informasi kelas dan pertemuan, contoh format "Kelas A Pertemuan ke 1 Cost Accounting Pengantar Cost Accounting"')); ?>
